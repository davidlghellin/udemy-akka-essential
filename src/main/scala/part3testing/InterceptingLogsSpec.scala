...
object InterceptingLogsSpec {
  ...
  // tienda, flujo de pago
  /*
  Así que la caída será así cuando desee enviar un mensaje a este actor de verificación.
  actor validará su tarjeta de pago con el administrador de pagos.
  El administrador de pagos responderá al agente de salida con un pago aceptado o denegado.
  Y si se acepta el pago, el agente de salida hablará con el gerente de cumplimiento para enviar su
  orden y luego, cuando el actor de pago recibe una respuesta del gerente de cumplimiento, el actor de pago
  volverá a su estado normal, donde también puede aceptar otros comandos de pago.
   */
